...
class JHSBrand():
                ...
                for i in range(0,len(activities)):
                    ...
                    if int(b_position_start+i) >= 0:
                        ...
                        if b.brandact_sign != 3:
                            ...
                            # Activity Items
                            # 多线程
                            m_items_obj = JHSItemM()
                            itemnum = self.activityItems(b.brandact_id, b.brandact_name, b.brandact_url, m_itemsObj)
                            # 多线程抓取
                            m_itemsObj.crawl()
                            self.item_queue.put((b.brandact_id,b.brandact_name,m_itemsObj))
                            #m_itemObj_list.append((b.brandact_id,b.brandact_name,m_itemsObj))
    ...
    # 从品牌团页获取数据
    def activityItems(self, actId, actName, actUrl, m_items_obj):
        allnum = 0
        ...
        if m:
            allnum = self.activityType1(page, actId, actName, actUrl, m_items_obj)
        else:
            m = re.search(r'<div class="act-main ju-itemlist">', page, flags=re.S)
            if m:
                allnum = self.activityType2(m.group(1), actId, actName, actUrl, m_items_obj)
            else:
                m = re.search(r'<div class="ju-itemlist J_JuHomeList">\s+<ul.+?>(.+?)</ul>', page, flags=re.S)
                if m:
                    allnum = self.activityType3(m.group(1), actId, actName, actUrl, m_items_obj)
                else:
                    m = re.search(r'<div class="l-floor J_Floor .+?data-ajaxurl="(.+?)">', page, flags=re.S)
                    if m:
                        allnum = self.activityType4(page, actId, actName, actUrl, m_items_obj)
                    else:
                        allnum = self.activityTypeOther(page, actId, actName, actUrl, m_items_obj)
        return allnum

    # 品牌团页面格式(1)
    def activityType1(self, page, actId, actName, actUrl, m_items_obj):
        position = 0
        # source html floor
        # 第一层
        ...
        for floor_url in p.finditer(page):
            ...
            print f_url
            position = self.getItemDataFromInterface(f_url, actId, actName, actUrl, position, m_items_obj)
        return position

    # 品牌团页面格式(2)
    def activityType2(self, page, actId, actName, actUrl, m_items_obj):
        position = 0
        # source html floor
        # 第一层
    ...
    # 品牌团页面格式(3)
    def activityType3(self, page, actId, actName, actUrl, m_items_obj):
        position = 0
    ...
    # 品牌团页面格式(4)
    def activityType4(self, page, actId, actName, actUrl, m_items_obj):
        position = 0
        # 请求接口数据
    ...
    # 品牌团页面格式
    def activityTypeOther(self, page, actId, actName, actUrl, m_items_obj):
        position = 0
        items = {}
    ...
    # 从接口获取数据
    def getItemDataFromInterface(self, url, actId, actName, actUrl, position, m_items_obj):
        ts = str(int(time.time()*1000)) + '_' + str(random.randint(0,9999))
